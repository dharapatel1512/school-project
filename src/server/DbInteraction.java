    ...
    public ArrayList<Member> buildQuery() throws SQLException {
        ...
        switch(members.get(0).getMessage()) {
            case "login" :
                ...
                break;
            case "getUser" :
                //ps=conn.prepareStatement("SELECT * From member_details.Members WHERE username=?");
                ps=conn.prepareStatement("SELECT * From Members WHERE username=?");
            ...
            case "add" :
                ...
                if(rs2!=0) {
                    System.out.println("Number of rows affected:"+ rs2);
                }else {
                    System.out.println("Number of rows affected:"+ rs2 +"Please try again");
                }
